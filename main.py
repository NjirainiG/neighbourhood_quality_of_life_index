...
try:
    if not firebase_admin._apps:
        config_path = st.secrets["FIREBASE_CONFIG"]
        if not os.path.exists(config_path):
            raise GeoAIError(f"Firebase config file not found at: {config_path}", 404)
        ...
        firebase_admin.get_app('geoai-app')
except Exception as e:
    st.error(f"Firebase initialization failed: {str(e)}")
    st.stop()

# Get bucket from initialized app
bucket = storage.bucket(app=firebase_admin.get_app('geoai-app'))
